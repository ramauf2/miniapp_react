...
export function ReferralsModal({ onClose, authData }) {
                        ...
                        <div className="space-y-2">
                            {referalHistory.map((earning, index) => (
                                <div key={index} className="bg-[#303030] rounded-[12px] p-3 flex items-center justify-between">
                                    ...
                                    <span className="text-[#00A61E] text-[16px] font-semibold flex items-center gap-1">
                                        +{(parseInt(earning.reward) / 1000000000).toFixed(2)} <TonIcon size={18} />
                                    </span>
                                </div>
                            ))}
                        </div>
