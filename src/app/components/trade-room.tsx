...
export function TradeRoom({ socket, authData, tradeData, goBack, localBalance = '0' }: TradeRoomProps) {
    const [partnerView, setPartnerView] = useState<'trade' | 'inventory'>('trade');
    const [isGiftModalOpen, setIsGiftModalOpen] = useState(false);
    const [selfAvatar, setSelfAvatar] = useState(authData.avatar || BASE_PATH + "/images/b40069be8827d28186f71205316130af8e40fdf6.png");
    const [partnerAvatar, setPartnerAvatar] = useState(BASE_PATH + "/images/94092276aedfc9cc35894c4b5622d96bbb2d09ba.png");
    ...
    useEffect(() => {
        Gifts.getUserGifts(authData['bearerToken']).then(data => {
            if (data.success) {
                setSelfTotalGifts(data.data.self.filter(i => (i.status == 'active' || i.status == 'in_trade')));
                setPartnerTotalGifts(data.data.partner.filter(i => (i.status == 'active' || i.status == 'in_trade')));
                setIsGiftsLoaded(true);
            }
        })
        
        // Загружаем аватарку партнера
        const partnerUsername = tradeData.isCreator ? tradeData.partner : tradeData.user;
        fetch(`https://t.me/${partnerUsername}`)
            .then(() => {
                setPartnerAvatar(`https://t.me/i/userpic/320/${partnerUsername}.jpg`);
            })
            .catch(() => {
                setPartnerAvatar(BASE_PATH + "/images/94092276aedfc9cc35894c4b5622d96bbb2d09ba.png");
            });
    }, []);
    ...
    /**
     * Открыть модальное окно для подтверждения удаления подарка
     */
    const handleRemoveGiftClick = (gift: Gift) => {
        if (isSelfAccepted) {
            return;
        }
    ...
    return (
        <>
            ...
            <div className="bg-[#303030] rounded-[20px] p-4 mb-6">
                ...
                >
                    ...
                    <div className="relative flex">
                        ...
                        >
                            <img src={selfAvatar} alt="user" className="w-[24px] h-[24px] rounded-full object-cover" />
                        ...
                        >
                            <img src={partnerAvatar} alt="partner" className="w-[24px] h-[24px] rounded-full object-cover" />
                ...
                <div>
                    <p className="text-[#999] text-[15px] mb-2 flex items-center gap-1">
                        {activeUser === 'self' ? 'Add' : 'Partner'} <TonIcon size={16} /> to trade
                    </p>
            ...
            {showAcceptModal && (
                <TradeConfirmModal
                    title="Принять обмен?"
                    message="После принятия ваши подарки будут заморожены и вы не сможете их изменить."
                    confirmText="Принять"
                    cancelText="Отмена"
                    onConfirm={confirmAcceptTrade}
            ...
            {showCancelModal && (
                <TradeConfirmModal
                    ...
                    message="Вы уверены, что хотите отменить этот обмен? Весь прогресс будет потерян."
