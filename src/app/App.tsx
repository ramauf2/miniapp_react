...
export default function App() {
    ...
    useEffect(() => {
        if (!ui && typeof window.TonConnectUI !== 'undefined') {
            setUi(
                new TonConnectUI({
                    manifestUrl: BASE_PATH + '/tonconnect-manifest.json',
                    buttonRootId: 'ton-connect',
                })
            );
        } else if (!ui && typeof window.TonConnectUI === 'undefined') {
            console.error('TonConnectUI не загружен! Проверьте скрипты в index.html');
        }
    }, [ui]);
    ...
    useEffect(() => {
        ...
        if (!isAuthAttempt) {
            ...
            if (localStorage.getItem('telegramId')) {
                ...
                User.login(ref, window.Telegram?.WebApp?.initData).then((data: any) => {
                    if (data.success) {
    ...
    const handleSocketTradeUpdated = (data: any) => {
        if (data.actor == authRef.current?.username) return;
        requestTradeInfo(data.code);
    };
    const handleSocketJoinTrade = (data: any) => {
    ...
    const handleSocketBalanceUpdated = (data: any) => {
        requestUserBalance();
    }
    ...
    const openTradeLink = () => setCurrentScreen('trade-link');
    ...
    const handleSendLink = () => setCurrentScreen('home');
    const handleCancelDeposit = () => setCurrentScreen('home');
    const handleOpenReferrals = () => setCurrentScreen('referrals');
    ...
    const renderScreen = () => {
        ...
        if (activeTab === 'profile' && authData) {
            ...
            return <Profile authData={authData} onOpenReferrals={handleOpenReferrals} socket={socketRef.current} />;
        }
